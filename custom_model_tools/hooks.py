...
class GlobalMaxPool2d:
    ...
    def hook(cls, activations_extractor):
        ...
        assert not cls.is_hooked(
            activations_extractor
        ), "GlobalMaxPool2d already hooked"
    ...
    def is_hooked(cls, activations_extractor):
        return any(
            isinstance(hook, cls)
            for hook in activations_extractor._extractor._batch_activations_hooks.values()
        )
...
class GlobalAvgPool2d:
    ...
    def hook(cls, activations_extractor):
        ...
        assert not cls.is_hooked(
            activations_extractor
        ), "GlobalAvgPool2d already hooked"
        handle = activations_extractor.register_batch_activations_hook(hook)
        hook.handle = handle
        return handle
...
class RandomProjection:
    ...
    def __call__(self, batch_activations):
        def apply(layer, activations):
            ...
            if layer not in self.layer_ws:
                w = np.random.normal(
                    size=(activations.shape[-1], self.n_components)
    ...
    def hook(cls, activations_extractor, n_components=1024):
        hook = RandomProjection(
            activations_extractor=activations_extractor, n_components=n_components
        )
        assert not cls.is_hooked(
            activations_extractor
        ), "RandomProjection already hooked"
        handle = activations_extractor.register_batch_activations_hook(hook)
        hook.handle = handle
        return handle
...
class LayerZCA:
    ...
    def _ensure_initialized(self, layers):
        ...
        if len(missing_layers) == 0:
            return
        layer_zcas = self._zcas(
            identifier=self._extractor.identifier, layers=missing_layers
        )
    ...
    def _zcas(self, identifier, layers):
        ...
        imagenet_activations = {
            layer: imagenet_activations.sel(layer=layer).values
        ...
        assert (
            len(
                set(
                    activations.shape[0]
                    for activations in imagenet_activations.values()
                )
            )
            == 1
        ), "stimuli differ"
