...
def fit_encoder(benchmark, model, layers, pooling, hooks=None):
    ...
    for layer in layers:
        if pooling:
            ...
            model.identifier = (
                model_identifier + f"|layer:{layer}|pooling:False|n_components:1000"
            )
        ...
        model_scores = LayerScores(
            model_identifier=model.identifier, activations_model=model, visual_degrees=8
        )
        ...
        if "aggregation" in score.dims:
            score = (
                score.to_dataframe(name="").unstack(level="aggregation").reset_index()
            )
...
def get_benchmark(benchmark, region, regression, data_dir):
    if benchmark == "majajhong2015":
        ...
        benchmark = Object2VecEncoderBenchmark(
            data_dir=data_dir, regions=regions, regression=regression
        )
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fit encoding models to a neural dataset"
    )
    parser.add_argument(
        "--bench",
        type=str,
        default="majajhong2015",
